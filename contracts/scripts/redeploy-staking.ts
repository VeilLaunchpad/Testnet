...
/**
 * Replaces DevoxStaking, keeping the token and the treasury.
 *
...
function plans(devox: string, isMainnet: boolean): PoolPlan[] {
  ...
  const list: PoolPlan[] = [
    ...
    {
      key: "DEVOX", token: devox, apyBps: 1800,
...
async function main() {
  ...
  const devoxAddress = process.env["NEXT_PUBLIC_DEVOX_TOKEN_" + suffix] || "";
  const treasuryAddress = process.env["NEXT_PUBLIC_DEVOX_TREASURY_" + suffix] || "";
  const oldStaking = process.env["NEXT_PUBLIC_DEVOX_STAKING_" + suffix] || "";
  if (!devoxAddress || !treasuryAddress) throw new Error("token or treasury missing for " + netKey);
  ...
  if (oldStaking) {
    ...
    for (const p of plans(devoxAddress, isMainnet)) {
      if (!p.token) continue;
  ...
  const factory = await ethers.getContractFactory("DevoxStaking");
  const args = [devoxAddress, treasuryAddress, deployer.address] as const;
  ...
  console.log("\n  DevoxStaking           " + stakingAddress + "  gas " + BigInt(estimate).toLocaleString("en-US"));
  ...
  console.log("  new contract approved, budget " + ethers.formatUnits(allocation, 18) + " DEVOX");
  ...
  writeEnv({ ["NEXT_PUBLIC_DEVOX_STAKING_" + suffix]: stakingAddress });
...
function patchTable(
  ...
  if (table?.contracts?.devoxpad?.devoxStaking) {
    table.contracts.devoxpad.devoxStaking.address = stakingAddress;
  }
  if (table.staking) {
